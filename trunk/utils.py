...
def GetOrCreateUserByIMEI(uimei, uphone = None, udesc = None):
	if not uimei: return None

	needupdate = False

	#userdb = db.get(db.Key("IMEI%s" % uimei))
	userdb = datamodel.DBUser().get_by_key_name("IMEI_%s" % uimei)
	if not userdb:
		userdbq = datamodel.DBUser().all().filter('imei =', uimei).fetch(1)
		if userdbq:
			...
			userdb = datamodel.DBUser(key_name="IMEI_%s" % uimei)
			userdb.imei = uimei
			userdb.phone = u'Неизвестно'
